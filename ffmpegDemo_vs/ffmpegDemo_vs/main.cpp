...
int main() {
    ...
    avformat_alloc_output_context2(&outCtx, NULL, NULL, dstFilename);
    if (!outCtx) {
    ...
    if (!stream_mapping) {
        fprintf(stderr, "Could not av_calloc\n");
        return -1;
    }
    ...
    for (int i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        out_stream = avformat_new_stream(outCtx, NULL);
        if (!out_stream) {
    ...
    int ret = avio_open(&outCtx->pb, dstFilename, AVIO_FLAG_WRITE);
    if (ret < 0) {
